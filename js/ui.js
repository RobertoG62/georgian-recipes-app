...
const UI = (() => {
    // Georgian cuisine categories with icons
    const CATEGORY_ICONS = {
        // Hebrew
        'הכל': 'fa-utensils',
        'חצ׳פורי ומאפים': 'fa-bread-slice',
        'חינקלי ובשרים': 'fa-drumstick-bite',
        'פחלי ואגוזים': 'fa-seedling',
        'מרקים ותבשילים': 'fa-bowl-food',
        'קינוחים ומשקאות': 'fa-wine-bottle',
        // English
        'All': 'fa-utensils',
        'Khachapuri & Breads': 'fa-bread-slice',
        'Khinkali & Meats': 'fa-drumstick-bite',
        'Pkhali & Walnut Dishes': 'fa-seedling',
        'Soups & Stews': 'fa-bowl-food',
        'Sweets & Drinks': 'fa-wine-bottle',
    };
    ...
    function renderCards(recipes) {
        ...
        grid.innerHTML = recipes.map(recipe => `
            <article class="recipe-card card-lift" data-id="${recipe.id}">
                ...
                <div class="p-4">
                    <h3 class="font-bold text-lg mb-0.5 text-ge-charcoal">${recipe.title}</h3>
                    ${recipe.originalName ? `<p class="text-ge-primary-dark text-xs italic mb-2 font-playfair">${recipe.originalName}</p>` : ''}
                    <p class="text-ge-text-secondary text-sm line-clamp-2 mb-3">${recipe.description}</p>
                    <div class="flex items-center justify-between text-xs text-ge-text-secondary">
            ...
            <div class="view-fade-in">
                <div class="recipe-hero" style="background-image: url('${recipe.image}')">
                    <div class="relative z-10 w-full p-6 md:p-10 max-w-4xl mx-auto">
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-ge-primary text-white mb-3">
                        ...
                        ${recipe.originalName ? `<p class="text-ge-primary-light text-base md:text-lg italic font-playfair mt-1">${recipe.originalName}</p>` : ''}
                ...
                <div class="max-w-4xl mx-auto px-4 -mt-8 relative z-10">
                    <div class="glass rounded-xl p-4 flex justify-center gap-0 mb-8">
                        <div class="meta-item">
                            <i class="far fa-clock text-ge-primary"></i>
                            <span class="font-bold text-ge-charcoal">${recipe.prepTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-ge-text-secondary">${i18n.t('detail.prepTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-fire text-ge-primary"></i>
                            <span class="font-bold text-ge-charcoal">${recipe.cookTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-ge-text-secondary">${i18n.t('detail.cookTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-users text-ge-primary"></i>
                            <span class="font-bold text-ge-charcoal">${recipe.servings}</span>
                            <span class="text-xs text-ge-text-secondary">${i18n.t('detail.servings')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-gauge text-ge-primary"></i>
                            <span class="font-bold text-ge-charcoal">${recipe.difficulty}</span>
                            <span class="text-xs text-ge-text-secondary">${i18n.t('detail.difficulty')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas ${getKosherBadge(recipe.kosherType).icon} text-ge-primary"></i>
                            <span class="font-bold text-ge-charcoal">${recipe.kosherType}</span>
                            <span class="text-xs text-ge-text-secondary">${i18n.t('detail.kosher')}</span>
                        </div>
                    </div>

                    <div class="glass gold-border-right rounded-xl p-6 mb-8">
                        <h2 class="font-bold text-xl mb-4 text-ge-charcoal flex items-center gap-2">
                            <i class="fas fa-list text-ge-primary"></i>
                            ${i18n.t('detail.ingredients')}
                            <span class="text-sm font-normal text-ge-text-secondary">(${recipe.ingredients.length})</span>
                        </h2>
                        <div>
                            ${recipe.ingredients.map(ing => `
                                <div class="ingredient-row">
                                    <span class="text-ge-primary"><i class="fas fa-check text-xs"></i></span>
                    ...
                    <div class="mb-8">
                        <h2 class="font-bold text-xl mb-6 text-ge-charcoal flex items-center gap-2">
                            <i class="fas fa-list-ol text-ge-primary"></i>
                        ...
                        <div class="space-y-4">
                            ${recipe.instructions.map((step, i) => `
                                <div class="flex gap-4 items-start">
                                    ...
                                    <p class="text-ge-charcoal pt-1 leading-relaxed">${mdBold(step)}</p>
                                </div>
                            `).join('')}
                        </div>
                    </div>
